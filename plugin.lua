...
function OnSetText(uri, text)
	...
	local isManifest = str_find(uri, 'fxmanifest%.lua$') or str_find(uri, '__resource%.lua$')

	-- Safe navigation is the only other thing this rewrites, and a plain search
	-- for '?' is far cheaper than the two pattern scans it guards. This function
	-- runs on every edit of every Lua file in the workspace, and the overwhelming
	-- majority of them contain no '?' at all.
	local hasSafeNav = str_find(text, '?', 1, true) ~= nil

	-- Nothing to rewrite: return nil rather than an empty table, so a file that
	-- needs no work costs no allocation.
	if not isManifest and not hasSafeNav then return end
